...
interface SchulungsanmeldungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Schulungsanmeldung['fields']) => Promise<void>;
  defaultValues?: Schulungsanmeldung['fields'];
...
export function SchulungsanmeldungDialog({ open, onClose, onSubmit, defaultValues, mitarbeiterList, schulungstermineList, enablePhotoScan = false, enablePhotoLocation = true }: SchulungsanmeldungDialogProps) {
  const [fields, setFields] = useState<Partial<Schulungsanmeldung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'schulungsanmeldung');
      await onSubmit(clean as Schulungsanmeldung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["mitarbeiter", "schulungstermin"]);
        ...
        return merged as Partial<Schulungsanmeldung['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Schulungsanmeldung bearbeiten' : 'Schulungsanmeldung hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="mitarbeiter">Ihr Name</Label>
            <Select
          ...
          <div className="space-y-2">
            <Label htmlFor="schulungstermin">Gewünschter Schulungstermin</Label>
            <Select
