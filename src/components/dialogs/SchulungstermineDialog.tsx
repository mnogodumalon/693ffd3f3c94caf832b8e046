...
interface SchulungstermineDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Schulungstermine['fields']) => Promise<void>;
  defaultValues?: Schulungstermine['fields'];
...
export function SchulungstermineDialog({ open, onClose, onSubmit, defaultValues, schulungskatalogList, trainerList, raeumeList, enablePhotoScan = false, enablePhotoLocation = true }: SchulungstermineDialogProps) {
  const [fields, setFields] = useState<Partial<Schulungstermine['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'schulungstermine');
      await onSubmit(clean as Schulungstermine['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["schulung", "trainer", "raum"]);
        ...
        return merged as Partial<Schulungstermine['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Schulungstermine bearbeiten' : 'Schulungstermine hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="startdatum">Startdatum und -uhrzeit</Label>
            <Input
              id="startdatum"
              type="datetime-local"
              step="60"
          ...
          <div className="space-y-2">
            <Label htmlFor="verfuegbare_plaetze">Verfügbare Plätze</Label>
            <Input
              id="verfuegbare_plaetze"
              type="number"
              value={fields.verfuegbare_plaetze ?? ''}
              onChange={e => setFields(f => ({ ...f, verfuegbare_plaetze: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <div className="space-y-2">
